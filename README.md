...
|-------|---------|-------------|
|| `listen_addr` | `:21432` | Port to listen on (use a different port if real Ollama is running) |
...
```bash
export OLLAMA_BASE_URL=http://localhost:21432
...
```json
{
  "provider": {
    "ollama": {
      ...
      "name": "Ollama Bridge",
      "options": {
        "baseURL": "http://localhost:21432/v1"
      },
      "models": {
...
```bash
export OPENAI_BASE_URL=http://localhost:21432/v1
